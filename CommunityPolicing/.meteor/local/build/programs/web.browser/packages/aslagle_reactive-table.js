    ...
    'sortedRows': function () {                                                                                      // 483
        if (this.server) {                                                                                           // 484
            ...
            var sortByValue = _.all(getSortedFields(this.fields, this.multiColumnSort), function (field) {           // 493
                return field.sortByValue || (!field.fn && !field.sortFn);                                            // 494
            });                                                                                                      // 495
            var filterQuery = getFilterQuery(getFilterStrings(this.filters.get()), getFilterFields(this.filters.get(), this.fields), {enableRegex: this.enableRegex, filterOperator: this.filterOperator});
                                                                                                                     // 497
...
sortWithFunctions = function (rows, fields, multiColumnSort) {                                                       // 83
  ...
  _.each(sortedFields.reverse(), function (field) {                                                                  // 87
    if (field.sortFn) {                                                                                              // 88
      sortedRows = _.sortBy(sortedRows, function (row) {                                                             // 89
        return field.sortFn( get( row, field.key ), row );                                                           // 90
      });                                                                                                            // 91
    } else if (field.sortByValue || !field.fn) {                                                                     // 92
      sortedRows = _.sortBy(sortedRows, field.key);                                                                  // 93
    } else {                                                                                                         // 94
      sortedRows = _.sortBy(sortedRows, function (row) {                                                             // 95
        return field.fn( get( row, field.key ), row );                                                               // 96
      });                                                                                                            // 97
    }                                                                                                                // 98
    if (field.sortDirection.get() === -1) {                                                                          // 99
      sortedRows.reverse();                                                                                          // 100
    }                                                                                                                // 101
  });                                                                                                                // 102
  return sortedRows;                                                                                                 // 103
};                                                                                                                   // 104
                                                                                                                     // 105
getPrimarySortField = function (fields, multiColumnSort) {                                                           // 106
  return getSortedFields(fields, multiColumnSort)[0];                                                                // 107
};                                                                                                                   // 108
                                                                                                                     // 109
changePrimarySort = function(fieldId, fields, multiColumnSort) {                                                     // 110
  var primarySortField = getPrimarySortField(fields, multiColumnSort);                                               // 111
  if (primarySortField && primarySortField.fieldId === fieldId) {                                                    // 112
    var sortDirection = -1 * primarySortField.sortDirection.get();                                                   // 113
    primarySortField.sortDirection.set(sortDirection);                                                               // 114
    primarySortField.sortOrder.set(0);                                                                               // 115
  } else {                                                                                                           // 116
    _.each(fields, function (field) {                                                                                // 117
      if (field.fieldId === fieldId) {                                                                               // 118
        field.sortOrder.set(0);                                                                                      // 119
        if (primarySortField) {                                                                                      // 120
          field.sortDirection.set(primarySortField.sortDirection.get());                                             // 121
        }                                                                                                            // 122
      } else {                                                                                                       // 123
        var sortOrder = 1 + field.sortOrder.get();                                                                   // 124
        field.sortOrder.set(sortOrder);                                                                              // 125
      }                                                                                                              // 126
    });                                                                                                              // 127
  }                                                                                                                  // 128
};                                                                                                                   // 129
...
getFilterQuery = function (filterInputs, filterFields, settings) {                                                   // 49
      ...
      if (_.any(settings.fields, function (include) { return include; })) {                                          // 59
        filterFields[index] = _.filter(filterFields[index], function (field) {                                       // 60
          return _.any(getFieldMatches(field), function (fieldMatch) {                                               // 61
            // ensure that the _id field is filtered on, even if it is not explicitly mentioned                      // 62
            if (fieldMatch === "_id") return true;                                                                   // 63
            return settings.fields[fieldMatch];                                                                      // 64
          });                                                                                                        // 65
        });                                                                                                          // 66
      } else {                                                                                                       // 67
        filterFields[index] = _.filter(filterFields[index], function (field) {                                       // 68
          return _.all(getFieldMatches(field), function (fieldMatch) {                                               // 69
            return _.isUndefined(settings.fields[fieldMatch]) || settings.fields[fieldMatch];                        // 70
          });                                                                                                        // 71
        });                                                                                                          // 72
      }                                                                                                              // 73
    });                                                                                                              // 74
  }                                                                                                                  // 75
  var numberRegExp = /^\d+$/;                                                                                        // 76
  var queryList = [];                                                                                                // 77
  _.each(filterInputs, function (filter, index) {                                                                    // 78
    if (filter) {                                                                                                    // 79
      if (_.isObject(filter)) {                                                                                      // 80
        var fieldQueries = _.map(filterFields[index], function (field) {                                             // 81
          var query = {};                                                                                            // 82
          query[field] = filter;                                                                                     // 83
          return query;                                                                                              // 84
        });                                                                                                          // 85
        if (fieldQueries.length) {                                                                                   // 86
            queryList.push({'$or': fieldQueries});                                                                   // 87
          }                                                                                                          // 88
      } else {                                                                                                       // 89
        var filters = parseFilterString(filter);                                                                     // 90
        _.each(filters, function (filterWord) {                                                                      // 91
          if (settings.enableRegex === false) {                                                                      // 92
            filterWord = escapeRegex(filterWord);                                                                    // 93
          }                                                                                                          // 94
          var filterQueryList = [];                                                                                  // 95
          _.each(filterFields[index], function (field) {                                                             // 96
            var filterRegExp = new RegExp(filterWord, 'i');                                                          // 97
            var query = {};                                                                                          // 98
            query[field] = filterRegExp;                                                                             // 99
            filterQueryList.push(query);                                                                             // 100
                                                                                                                     // 101
            if (numberRegExp.test(filterWord)) {                                                                     // 102
              var numberQuery = {};                                                                                  // 103
              numberQuery[field] = parseInt(filterWord, 10);                                                         // 104
              filterQueryList.push(numberQuery);                                                                     // 105
            }                                                                                                        // 106
                                                                                                                     // 107
            if (filterWord === "true") {                                                                             // 108
              var boolQuery = {};                                                                                    // 109
              boolQuery[field] = true;                                                                               // 110
              filterQueryList.push(boolQuery);                                                                       // 111
            } else if (filterWord === "false") {                                                                     // 112
              var boolQuery = {};                                                                                    // 113
              boolQuery[field] = false;                                                                              // 114
              filterQueryList.push(boolQuery);                                                                       // 115
            }                                                                                                        // 116
          });                                                                                                        // 117
                                                                                                                     // 118
          if (filterQueryList.length) {                                                                              // 119
            var filterQuery = {'$or': filterQueryList};                                                              // 120
            queryList.push(filterQuery);                                                                             // 121
          }                                                                                                          // 122
        });                                                                                                          // 123
      }                                                                                                              // 124
    }                                                                                                                // 125
  ...
  var query = {};                                                                                                    // 128
                                                                                                                     // 129
  if(queryList.length) {                                                                                             // 130
    query[settings.filterOperator] = queryList;                                                                      // 131
  }                                                                                                                  // 132
                                                                                                                     // 133
  return query;                                                                                                      // 134
};                                                                                                                   // 135
...
if (Meteor.isClient) {                                                                                               // 137
  ReactiveTable = ReactiveTable || {};                                                                               // 138
                                                                                                                     // 139
  var reactiveTableFilters = {};                                                                                     // 140
  var callbacks = {};                                                                                                // 141
                                                                                                                     // 142
  ReactiveTable.Filter = function (id, fields) {                                                                     // 143
    if (reactiveTableFilters[id]) {                                                                                  // 144
      reactiveTableFilters[id].fields = fields;                                                                      // 145
      return reactiveTableFilters[id];                                                                               // 146
    }                                                                                                                // 147
                                                                                                                     // 148
    var filter = new ReactiveVar();                                                                                  // 149
                                                                                                                     // 150
    this.fields = fields;                                                                                            // 151
                                                                                                                     // 152
    this.get = function () {                                                                                         // 153
      return filter.get() || '';                                                                                     // 154
    };                                                                                                               // 155
                                                                                                                     // 156
    this.set = function (filterString) {                                                                             // 157
      filter.set(filterString);                                                                                      // 158
      _.each(callbacks[id], function (callback) {                                                                    // 159
        callback();                                                                                                  // 160
      });                                                                                                            // 161
    };                                                                                                               // 162
                                                                                                                     // 163
    reactiveTableFilters[id] = this;                                                                                 // 164
  };                                                                                                                 // 165
                                                                                                                     // 166
  ReactiveTable.clearFilters = function (filterIds) {                                                                // 167
    _.each(filterIds, function (filterId) {                                                                          // 168
      if (reactiveTableFilters[filterId]) {                                                                          // 169
        reactiveTableFilters[filterId].set('');                                                                      // 170
      }                                                                                                              // 171
    });                                                                                                              // 172
  };                                                                                                                 // 173
                                                                                                                     // 174
  dependOnFilters = function (filterIds, callback) {                                                                 // 175
    _.each(filterIds, function (filterId) {                                                                          // 176
      if (_.isUndefined(callbacks[filterId])) {                                                                      // 177
        callbacks[filterId] = [];                                                                                    // 178
      }                                                                                                              // 179
      callbacks[filterId].push(callback);                                                                            // 180
    });                                                                                                              // 181
  };                                                                                                                 // 182
                                                                                                                     // 183
  getFilterStrings = function (filterIds) {                                                                          // 184
    return _.map(filterIds, function (filterId) {                                                                    // 185
      if (_.isUndefined(reactiveTableFilters[filterId])) {                                                           // 186
        reactiveTableFilters[filterId] = new ReactiveTable.Filter(filterId);                                         // 187
      }                                                                                                              // 188
      return reactiveTableFilters[filterId].get();                                                                   // 189
    });                                                                                                              // 190
  };                                                                                                                 // 191
                                                                                                                     // 192
  getFilterFields = function (filterIds, allFields) {                                                                // 193
    return _.map(filterIds, function (filterId) {                                                                    // 194
      if (_.isUndefined(reactiveTableFilters[filterId])) {                                                           // 195
        return _.map(allFields, function (field) { return field.key; });                                             // 196
      } else if (_.isEmpty(reactiveTableFilters[filterId].fields)) {                                                 // 197
        return _.map(allFields, function (field) { return field.key; });                                             // 198
      } else {                                                                                                       // 199
        return reactiveTableFilters[filterId].fields;                                                                // 200
      }                                                                                                              // 201
    });                                                                                                              // 202
  };                                                                                                                 // 203
                                                                                                                     // 204
  Template.reactiveTableFilter.helpers({                                                                             // 205
    'class': function () {                                                                                           // 206
      return this.class || 'input-group';                                                                            // 207
    },                                                                                                               // 208
                                                                                                                     // 209
    'filter': function () {                                                                                          // 210
      if (_.isUndefined(reactiveTableFilters[this.id])) {                                                            // 211
        new ReactiveTable.Filter(this.id, this.fields);                                                              // 212
      } else if (_.isUndefined(reactiveTableFilters[this.id].fields)) {                                              // 213
        reactiveTableFilters[this.id].fields = this.fields;                                                          // 214
      }                                                                                                              // 215
      return reactiveTableFilters[this.id].get();                                                                    // 216
    }                                                                                                                // 217
  });                                                                                                                // 218
                                                                                                                     // 219
  var updateFilter = _.debounce(function (template, filterText) {                                                    // 220
    reactiveTableFilters[template.data.id].set(filterText);                                                          // 221
  }, 200);                                                                                                           // 222
                                                                                                                     // 223
  Template.reactiveTableFilter.events({                                                                              // 224
    'keyup .reactive-table-input, input .reactive-table-input': function (event) {                                   // 225
      var template = Template.instance();                                                                            // 226
      var filterText = $(event.target).val();                                                                        // 227
      updateFilter(template, filterText);                                                                            // 228
    },                                                                                                               // 229
  });                                                                                                                // 230
}                                                                                                                    // 231
                                                                                                                     // 232
